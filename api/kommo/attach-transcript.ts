...
async function addNote(leadId: number, text: string) {
  ...
  if (!r.ok) throw new Error(`attach-transcript ${r.status}: ${await r.text()}`);
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!leadId || !transcript) return res.status(400).json({ error: 'lead_id and transcript required' });

    const header = `📎 Conversación completa${title ? ` — ${title}` : ''}\nFecha: ${new Date().toISOString()}`;
    await addNote(leadId, header);

    const CHUNK = 3500;
    for (let i = 0; i < transcript.length; i += CHUNK) {
      await addNote(leadId, transcript.slice(i, i + CHUNK));
    }
    return res.status(200).json({ ok: true, chunks: Math.ceil(transcript.length / CHUNK) });
  } catch (e: any) {
